...
public class StringUtil {
     ...
     * @return
     */
    public static SpannableString createColorText(String parent, int[] colors, int[] len, Object... items) {
        //拼装字段
        SpannableString span = new SpannableString(MessageFormat.format(parent, items));
        //颜色渲染
        ForegroundColorSpan colorSpan;
        int count = 0;
        for (int i = 0; i < items.length; i++) {
            colorSpan = new ForegroundColorSpan(colors[i]);
            span.setSpan(colorSpan, count + len[i], count + len[i] + items[i].toString().length()
                    , Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            count += len[i] + items[i].toString().length();
        }
        return span;
    }
    ...
    public static void emptyAll(EditText text) {
        text.setText(null);
    }
